...
def test_processing_overlay_names_the_generating_model(tmp_path):
    # The suggest/lesson overlay says WHICH engine is writing the content, so a long
    # wait is transparent ("generated by claude · <model>"), fed from /learn/state's
    # backend_info and refreshed on every state load.
    repo = _init_repo(tmp_path)
    html = learn.learn_html(repo.repo)
    assert 'id="ov-engine"' in html
    assert '"generated by " + info.backend' in html
    assert '$("ov-engine").textContent = engineLabel();' in html
    assert "state.backendInfo = d.backend_info" in html
