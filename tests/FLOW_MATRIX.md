...
| A message's own Markdown headings nest TWO levels under its role heading (`##` → `####`), shallowest normalized to level 4 with hierarchy preserved, never promoted, fenced code left verbatim | `test_trace_message_headings_are_nested_two_levels_under_role`, `test_a_message_heading_already_deep_enough_is_left_alone`, `test_trace_heading_nesting_skips_fenced_code_comments` | mock |
