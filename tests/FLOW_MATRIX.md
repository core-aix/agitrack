...
| The trace turn-limit counts TURNS, not `## User` blocks: messages queued mid-turn ride with the turn they continue (kept whole, or evicted whole), so a turn with more queued messages than the limit no longer loses its opening prompt | `test_a_message_queued_mid_turn_does_not_count_as_a_turn_of_its_own`, `test_a_turn_that_is_over_the_limit_is_still_dropped_whole`, `test_a_long_turns_queued_messages_survive_the_trace_limit` | mock |
