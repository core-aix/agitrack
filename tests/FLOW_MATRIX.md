...
| A MID-FLIGHT force capture anchors on the turn's USER id, never on the transient assistant id it carries at that instant (a backend mints a new message id per assistant response): anchoring on the transient id lost the whole turn once it continued | `test_a_midflight_capture_anchors_on_the_user_id_not_a_transient_assistant_id`, `test_finish_parse_exit_commits_dangling_no_text_turn` | mock |
