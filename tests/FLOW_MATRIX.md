...
| Sequence | Test(s) | Kind |
|---|---|---|
| Self-update is a real side effect (a source install FETCHES and MERGES the checkout), so the daemon watcher only does it on explicit opt-in and no test can reach the real updater (autouse conftest guard) — defaulting it on let a watcher test fast-forward CI's own checkout mid-run and fail the version test | `test_daemon_watcher_also_installs_updates` | mock |
