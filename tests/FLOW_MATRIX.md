...
| **A plain RESTART recovers what `--recover` does.** Startup only integrated work that was already COMMITTED, so after a SIGKILL the same crash left the finished turn committed+merged if the user ran `agitrack --recover` (what the editor extension does) and uncommitted behind "⚠ 1 stale session(s) need attention" if they simply relaunched. Startup now applies the same policy via `RecoveryService.recover_worktree` (it already holds the lock the standalone entry point would take); a turn still in flight is still never committed, only flagged | `test_a_plain_RESTART_recovers_what_agitrack_recover_would`, `test_a_restart_still_flags_a_turn_that_was_never_finished` | real-git |
| **…and it comes back as ITSELF.** Both records that answer "what do I resume?" die with a SIGKILL: the root resume pointer is written only by the graceful exit path, and the repo scan looks in the BASE project dir while a worktree session's conversation is keyed by the WORKTREE path. So a killed run relaunched as a brand-new randomly named session with a fresh conversation while the real one sat in `.agitrack/worktrees/<name>` — work intact, identity lost. Startup now falls back to `_newest_worktree_session()` (the same question `--no-worktree` already asks), which also restores the surviving `pending_session_name`; a recorded pointer still wins | `test_a_crashed_run_resumes_its_own_session_instead_of_starting_a_random_new_one`, `test_the_recorded_resume_pointer_still_wins_when_there_is_one` | real-git |
...
| A conversation NEVER inherits another conversation's watermark: a brand-new conversation (backend_session_id already reassigned, nothing of its own committed) used to read the PREVIOUS conversation's global id, which matched no boundary and had no marked_at, so `turns_after` fell to "newest turn only" and silently DISCARDED every earlier turn — prompt, tokens and trace in no commit at all | `test_a_new_conversation_never_inherits_the_previous_conversations_watermark`, `test_switching_back_reads_the_conversations_own_mark_not_the_global` | mock |
| ...while an explicit `last_backend_message_id = None` reset still clears the tracked conversation, and a global predating the per-conversation map (upgrade) is still honoured | `test_current_conversation_watermark_still_uses_single_value`, `test_a_watermark_predating_the_per_conversation_map_is_still_honoured` | mock |
| ...and a mark that genuinely BELONGS to this conversation but stopped matching a boundary still survives, so the bounded time fallback is reached rather than the whole conversation re-exported | `test_a_conversations_own_lost_watermark_still_reaches_the_bounded_fallback` | mock |
| A MID-FLIGHT force capture anchors on the turn's USER id, never on the transient assistant id it carries at that instant (a backend mints a new message id per assistant response): anchoring on the transient id truncated the turn to the partial snapshot once it continued — its real final message and remaining tokens recorded nowhere | `test_a_midflight_capture_anchors_on_the_user_id_not_a_transient_assistant_id`, `test_finish_parse_exit_commits_dangling_no_text_turn` | mock |
...
| A squashed merge shows EVERY pre-squash commit as its own item and keeps the merge's own message: constituents split on the squash bullet list when GitHub wrote one (blank line on BOTH sides — prose bullets do not qualify), else on the metadata blocks. A `# aGiTrack Metadata` line inside a FENCE is content, not a header, so a commit quoting one is neither mistaken for a squash nor read for its backend/model | `test_a_squash_shows_every_commit_as_its_own_item_and_keeps_its_own_message`, `test_a_summary_bullet_list_is_not_mistaken_for_a_squashed_commit`, `test_squash_parses_constituents_and_counts_their_tokens` | real-git |
| A committer whose commits carry no lines at all is dropped from the by-committer breakdown AND its filter (an empty row is not a person to filter by); one line anywhere keeps them, and co-authors are counted over every committer of a commit, not just its author | `test_a_committer_with_no_lines_is_dropped_from_the_breakdown_and_the_filter`, `test_a_committer_is_kept_when_any_of_their_commits_carries_lines` | mock |
...
| **…and the palette actually DELIVERS the typed argument.** It filtered on the whole string, so a space matched nothing, the filter fell back to the full command list, and Enter ran its first entry: `sessions 2` merely opened the picker, and with unmerged work present (which PREPENDS `merge`) it ran **merge**. `sessions <n>`, `sessions new` and `agent-backend <name>` were unreachable from the UI. Names containing a space (`exit aGiTrack`) are not mistaken for name+argument | `test_the_palette_keeps_a_typed_ARGUMENT_instead_of_running_another_command`, `test_the_palette_still_resolves_plain_commands_and_names_containing_a_space` | mock |
...
| A turn is an EXCHANGE and the trace limit counts those: the agent's reply closes a turn, so messages sent back-to-back with no answer between them are ONE turn (derived from the trace's shape, so it holds however they were recorded); a turn is kept whole or evicted whole, and a background event opens one too | `test_messages_sent_back_to_back_are_one_turn_not_several`, `test_a_message_queued_mid_turn_does_not_count_as_a_turn_of_its_own`, `test_a_turn_that_is_over_the_limit_is_still_dropped_whole`, `test_a_long_turns_queued_messages_survive_the_trace_limit` | mock |
| Every Markdown heading inside a trace message is pushed down exactly two levels (`#` → `###`, `##` → `####`) — a uniform shift, not a re-basing onto a floor, so the author's own hierarchy survives; capped at level 6; fenced code left verbatim | `test_trace_message_headings_are_pushed_down_two_levels`, `test_a_deeply_nested_message_heading_is_capped_at_six`, `test_trace_heading_nesting_skips_fenced_code_comments` | mock |
| A background wake-up is an EVENT in the trace, never a `## User` block the user never wrote: it renders as a note, is excluded from the commit subject (an all-background commit falls back to `<backend> changes`), and still counts as a turn START for the trace limit | `test_a_background_event_is_never_rendered_as_a_user_message`, `test_a_background_event_starts_a_turn_for_the_trace_limit`, `test_a_background_wake_up_is_not_attributed_to_the_user`, `test_a_real_prompt_alongside_a_background_wake_up_still_owns_the_subject` | mock |
...
| A tracker REMEMBERS its repo for the dashboard switcher itself, so a headless start (autotrack hook, script, no TTY, `open_dashboard_on_start` off) is listed even though it opens no dashboard; a registry that raises never takes the tracker down | `test_a_tracker_remembers_its_repo_for_the_dashboard_switcher`, `test_remembering_the_repo_never_breaks_the_tracker` | real-git |
| The switcher lists every repo with a LIVE daemon even if nothing remembered it, and remembers it as it adds it (so it outlives the tracker); a stopped repo is not resurrected by a surviving daemon; a vanished directory is not offered; an unreadable daemon registry still draws the remembered repos | `test_a_repo_with_a_live_tracker_is_offered_even_if_nothing_remembered_it`, `test_a_stopped_repo_is_not_resurrected_by_a_surviving_daemon`, `test_a_live_daemon_for_a_vanished_directory_is_not_offered`, `test_the_switcher_still_draws_when_the_daemon_registry_cannot_be_read` | real-git |
| A tracker whose repository is DELETED stops itself on the next poll (nothing else can reach it: stop works through files that went with the directory); removing `.git` alone does not, and a failed check never does | `test_a_tracker_stops_itself_once_its_repository_is_deleted`, `test_a_tracker_keeps_running_when_only_git_was_removed`, `test_a_tracker_never_stops_because_the_check_itself_failed` | real-git |
| The switcher orders repos MOST RECENTLY UPDATED first, from each repo's reflog mtime; a repo with no reflog falls back to `last_seen`; a tracker restart (which re-stamps `last_seen`) does not reshuffle the list | `test_repos_are_ordered_by_when_they_were_last_updated`, `test_a_repo_with_no_commits_is_ordered_by_when_it_was_last_worked_on`, `test_a_tracker_restart_does_not_reshuffle_the_switcher` | real-git |
| `running_repos` answers "which repos have a live daemon" from the daemon REGISTRY only — never a `ps` scan, since the switcher polls it — and excludes the hub, which belongs to no repo | `test_running_repos_reports_tracked_repos_without_a_process_scan`, `test_running_repos_excludes_the_hub_which_belongs_to_no_repo` | unit |
...
| **`-b --backend X` RECORDS X** (state file + global default) instead of only resolving it: the state was seeded with the same value, so the "differs?" test never fired and nothing was written. Downstream, the auto-track hook then asked `state.backend`, which RAISES with none recorded, straight into a blanket `except: return 0` — a silent no-op that lost the commit's AI work while `-s` still reported auto-start on; and the daemon the hook spawns is now told `--backend` explicitly (it resolves only the flag or the GLOBAL default, so it died on startup) | `test_a_background_run_records_the_backend_it_was_given`, `test_the_commit_hook_installs_as_soon_as_the_repo_state_names_a_backend`, `test_the_hook_names_the_backend_when_it_auto_starts_the_daemon` | real-git |
...
| **An OpenCode summarizer uses the SESSION's model, not OpenCode's global default.** The summarizer runs outside the repo by design, so the project's `opencode.json` pin cannot apply; with no `--model` OpenCode fell back to its global default and EVERY summary failed silently (exit 1, provider 403), leaving raw prompts as commit subjects while Claude sessions got real summaries. Claude deliberately still lets its CLI choose (a recorded model can carry a variant suffix `--model` rejects); a configured `summarization_model` still wins | `test_an_opencode_summarizer_uses_the_SESSION_model_not_opencodes_global_default`, `test_a_claude_summarizer_still_lets_the_cli_pick_its_own_default`, `test_a_configured_summarization_model_still_wins` | real-git |
| **…in the DAEMON too.** The proxy and the background tracker each build their own summarizer; fixing only the proxy left every `-b` OpenCode commit with a raw-prompt subject and `summarizer backend exited with 1` in `background.log` | `test_the_DAEMONS_summarizer_also_uses_the_session_model_on_opencode`, `test_the_DAEMONS_summarizer_still_lets_claude_choose` | real-git |
...
| One version string everywhere a person reads one (`versioning.version_line`): `agitrack --version`, the dashboard `/state` poll, the page and a crash report all print the release version plus the commit on a source checkout; machine-read fields (commit trailer, daemon registry, hook stamp) keep the bare version | `test_every_version_indicator_says_the_same_thing`, `test_version_flag_names_the_commit_on_a_source_checkout`, `test_the_running_check_also_carries_the_version_serving_it` | unit |
| A virtualenv with no pip is never upgraded through another interpreter's pip (it would install elsewhere and the diagnosis would name the wrong Python); the venv's own tool is named instead | `test_a_venv_with_no_pip_is_never_upgraded_through_another_interpreters_pip` | unit |
| The recorded verdict is a CACHE, re-checked against the aGiTrack running now: a record naming a version already installed, or one written by another instance of a different version, is not an action item — and SHAs are never compared as versions (`e4f5a6b` parses to `(0,)`, which suppressed every source checkout's notice) | `test_the_reminder_is_re_checked_against_the_agitrack_running_now`, `test_the_install_notice_speaks_versions_for_a_package_and_commits_for_a_source_checkout` | mock |
| On a repo dashboard the "install it yourself" notice speaks for THAT repo's instance (the install fingerprint in its repo lock), so a source-tracked repo never carries a wheel install's complaint; a repo with no instance still shows the global record | `test_a_repo_dashboard_only_shows_the_install_notice_of_its_own_instance` | mock |
| A page open in a browser reloads ITSELF when the daemon it polls comes back on a new version, and says why (the version rides the /state poll the running indicator already makes; the reason survives the reload in sessionStorage) | `test_a_page_left_open_reloads_itself_when_the_daemon_updates_under_it`, `test_the_running_check_also_carries_the_version_serving_it` | mock |
