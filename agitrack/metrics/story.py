...
/* Heading depth reads at a glance: structural sections (# …) brightest/largest,
   ...
   h5 and h6 are styled ALIKE apart from the indent: a message's own headings are
   nested to level 4+ by the commit renderer (TRACE_CONTENT_HEADING_LEVEL), so its
   PRIMARY sections arrive as h6 — they must read as content, not as the dimmest
   possible footnote. Commits written before that change put them at h5; both look
   the same here so history and new commits render consistently. */
...
.dmsg.md h6.md-h{font-size:12.5px;color:var(--ops);font-weight:500;padding-left:20px;border-left:2px solid var(--line)}
