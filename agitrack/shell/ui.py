...
class AgitrackPrompt:
    ...
    def _bottom_toolbar(self):
        ...
        hint = " | type : for aGiTrack controls, / for OpenCode controls"
        left = f" aGiTrack {state.backend} | {state.repo.name}{verbose}{hint}"
        if not right:
...
class _AgitrackCompleter(Completer):
    def get_completions(self, document, complete_event):
        ...
        if stripped.startswith(":"):
            yield from self._complete_commands(AGITRACK_COMMANDS, stripped, Completion)
