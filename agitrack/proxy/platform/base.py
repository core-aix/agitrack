...
class ChildProcess(Protocol):
    ...
    # ``write`` queues and never blocks: a PTY's input queue is ~1 KB and the reactor is
    # single-threaded, so blocking there stops it draining the child's OUTPUT and deadlocks
    # both sides. ``flush_input`` retries what the child would not take (True = still owed)
    # and ``pending_input`` reports how much is still queued.
