...
Press **write the story** and your backend agent lays out the chapters: aGiTrack groups the history into sittings of work first, so a few compact questions cover a whole repo (about two minutes for 780 commits). **A chapter is written out the first time you open it**, one call over that chapter's own material, so you only pay for depth where you actually read. Everything is stored per branch in `.agitrack/story.json` and costs nothing to reopen. When new commits arrive, **add the N new commits** continues the story instead of retelling it; **go further back** reaches deeper into old history. Tell it `plainly`, `playfully` or `epically`, and read it newest-first with **play the story**, `j`/`k`, or by linking straight to a chapter (`#ch-...`).
