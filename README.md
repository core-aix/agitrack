...
**Filter by anything the commits record.** Beside the committer / backend / model / date controls, an **advanced** panel filters on *any* field in a commit's aGiTrack metadata — harness version, operating system, session name, context compactions, token counts, whatever a future version adds. Pick a field, an operator (`is`, `contains`, `≥`, `≤` for numbers) and a value; add as many conditions as you like. Conditions on different fields all have to match, while several values of the *same* field match any of them, so you can widen one axis while narrowing another. The whole page — every panel, chart and the commit log — recomputes for the filtered slice, and the filter rides in the URL so it can be shared. It is offered on the backtrace view too.
