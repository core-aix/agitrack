...
1. **The parts.** Your history split into a handful of eras (how many depends on how long and how busy the history is, not a fixed five), each with its span, its size, and a sparkline of the work inside it. Click one to zoom in.
...
3. **Closer.** The same span re-told at finer grain, when a handful of moments is not enough.
