...
| **Long feedback loops** | Turns running over 30 minutes. Nothing can be steered mid-turn, so a long run that lands wrong is wasted whole. (Implausible spans — a resumed conversation stamps its whole length on one commit — are discarded, not reported.) | Ask for a plan or a first slice before a long autonomous run; delegate long sweeps to sub-agents that report back. |
