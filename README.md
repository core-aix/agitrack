...
**One dashboard, one port, every repository.** A single server serves all of your projects, switched by URL path: `/r/<repo>/` is a repository's tracked view and `/b/<repo>/` is its backtrace. Pick another repository from the selector in the page header and the same page you were reading (dashboard, story, or learn) opens for it. So there is one port to remember and one `ssh -L` line to forward, however many projects you have. The list of repositories it offers is the ones aGiTrack is **doing something for**: being tracked right now, or **armed** — nothing running this second, but the auto-start hook will start a tracker on your next commit or agent session (the row says `auto-start` rather than `off`). A repository that is neither, and that you have not opened here, is left out rather than cluttering the list forever; it is not forgotten, so working in it again — or `agitrack -d` inside it — brings it straight back. `agitrack stop` drops one from the list, and `agitrack -d stop` stops the whole dashboard.

**Filter by anything the commits record.** Beside the committer / backend / model / date controls, an **advanced** panel filters on *any* field in a commit's aGiTrack metadata — harness version, operating system, session name, context compactions, token counts, whatever a future version adds. Pick a field, an operator (`is`, `contains`, `between`, `≥`, `≤`) and a value; add as many conditions as you like, then press **apply** — nothing re-filters while you are still typing. Ranges work on more than numbers: `between` orders versions and timestamps sensibly too, so `2.1.220 → 2.1.238` and `2026-08-01 → 2026-08-20` both read the way you would expect, and either end can be left blank for an open range. Conditions on different fields all have to match, while several values of the *same* field match any of them, so you can widen one axis while narrowing another. The whole page — every panel, chart and the commit log — recomputes for the filtered slice, and the filter rides in the URL so it can be shared. It is offered on the backtrace view too.

**The header tells you what is happening.** A repository selector switches between every project aGiTrack knows, and every row in it carries that project's own tracking state: a lit dot and the mode where a tracker is running, "off" where none is, so one glance answers "which of my projects is actually being tracked?" for all of them at once. It scrolls when the list is long, re-reads the states each time it opens, and its last entry explains how to add a project. Beside it, a status light repeats the state of the repository you are looking at, and a **tracked / backtrace** toggle switches the view. Switching repository keeps the page you were reading and picks whichever view suits the project you moved to, rather than carrying the current one across.
...
**Choosing the coach engine.** By default lessons are generated by the **latest session's backend and model** for the repo. Change it either on the page (the "coach engine" panel lists every backend and the models each CLI reports) or in config: `learning_backend` / `learning_model` in `.agitrack/config.json` (repo) or `~/.agitrack/config.json` (global), repo value winning. A model id that belongs to the other backend's format is ignored so a mismatch can never break the page.
...
- **Codex's own sandbox is turned off while aGiTrack's is on.** Codex sandboxes each command with macOS `sandbox-exec`, and aGiTrack already wraps the whole agent in `sandbox-exec` to confine writes to the session worktree. Seatbelt sandboxes don't nest, so leaving both on made every filesystem probe fail and Codex fell back to asking permission for each command. aGiTrack disables the inner one *only* when its own confinement is active. Be aware of the trade-off: aGiTrack's profile is stricter **inside the repository** (the agent can only write its own session worktree), but it is *not* stricter outside it — Codex's own `workspace-write` sandbox restricts writes across the whole disk, and dropping it gives the agent **more** out-of-repo write reach than running Codex alone. Under `--no-sandbox`/`--no-confine`, Codex keeps its own sandbox.
...
**Other projects on your machine stay out of it.** A coding session rarely stays inside one project — "I ported this from acme-billing", "same bug as in client-portal/src/auth.ts" — and a commit message is published the moment the repository is pushed. Absolute paths were already masked (`/Users/you/Code/acme-billing/…` becomes `[PATH]`); on top of that, the **name** of every *other* repository aGiTrack knows about is replaced with `[OTHER_REPO]` in the subject, the summary and the interaction trace, including the trace folded into your own commits in `-m` mode. Paths into another project go whole, so `acme-billing/lib/retry.py` does not leave the file name behind. The agent is also told, in the note aGiTrack appends to its system prompt, to keep its final message about this repository.

Two deliberate limits. It is **best-effort, not a guarantee**: aGiTrack can only recognize names it knows — a project described rather than named, or one it has never been run in, still gets through. And it **never redacts an ordinary word**: a project called `paper`, `docs` or `notes` shares its name with words the conversation needed, and blanking every occurrence would cost the reader more than the leak ever did — so those names are left alone (a composed name like `my-paper` or `paper2024` is redacted normally). Set `redact_other_repos: false` in config to turn it off for a project whose sessions legitimately talk about its siblings.
...
- Interactive mode preserves the backend's selected model in commit metadata when it can be read from session data.
...
Each category is derived from the aGiTrack metadata already in your commits (per-turn prompts, token counts, session ids, timestamps, durations) plus the per-file change timeline the file browser builds. It is a single in-memory pass over data the dashboard already holds — no extra `git` calls, no LLM, a few milliseconds on a repo with hundreds of agent turns — so the panel appears with the rest of the page. It is part of the served page in both the live dashboard and [`--backtrace`](#backtrace--show-and-commit-a-history-you-didnt-track-from-day-one); the one-shot `-d text` report prints the raw metrics only.
...
| **Unverified code turns** | Turns that changed source files but touched no test alongside them — the upstream cause of both the loops and the hotspots. Silent in a repo (or a slice) with no tests, and docs-only turns never count. | Ask for the test in the same turn as the change, not the turn after. |
| **Rework hotspots** | Files the agent keeps **returning to within the hour** *and* churning in place — deletions roughly matching insertions, so the file barely grows despite heavy editing. Both conditions are required: a file edited often but steadily *growing* is several features being built (fine, not flagged); balanced add/delete is the same lines being redone. | State the full requirement in one prompt (or ask for a plan first), and pin the behaviour with a test so the next turn builds on the last. |
...
| **Overloaded turns** | Turns that change more than 8 files at once: several tasks bundled into one prompt, so the commit can't be reviewed or reverted as one decision. | One prompt per behaviour change; call out genuinely repo-wide sweeps explicitly. |
| **Long feedback loops** | Turns running over 30 minutes that you could have steered. Nothing can be corrected mid-turn, so a long run that lands wrong is wasted whole. Turns spent **waiting on a background task** (a build, a test run, a sub-agent) are excluded — that wait is expected, not a problem. (Implausible spans — a resumed conversation stamps its whole length on one commit — are also discarded, not reported.) | Ask for a plan or a first slice before a long autonomous run; delegate long sweeps to sub-agents that report back. |
...
Every finding carries the **numbers behind it** (counts, percentages, token totals, the files or prompts involved) rather than a bare verdict, and is ranked *high → medium → info*, with only the worst few expanded and the rest folded away.

**It follows the filter, so you can see yourself improve.** Insights are computed from exactly the commits the rest of the dashboard is showing. Narrow the **time range** (or a backend/model/committer) and the panel re-asks the question for that slice alone — which is the point: judged against your whole history forever, the bad old turns never leave the denominator and an improvement can never show up. Set the range to last week to see how last week actually went.

**Each card also carries its own trend.** Every category reduces to a single number where *lower is better*; aGiTrack recomputes it on the earlier and later half of whatever range you're viewing and shows the movement as a chip — `↓ 34% vs earlier half` (improving), `↑ 22% vs earlier half` (worsening), or `→ steady` inside a ±20% noise band. A habit that was a problem early in the range and has since dropped below its threshold is promoted to a green **"improved"** card instead of quietly disappearing, so a win is visible rather than merely being the absence of a warning.

The two halves are named explicitly, right under the section heading: *"Each trend compares the earlier half of this range, 2026-06-05 → 2026-06-19 (191 turns), with the later half, 2026-06-19 → 2026-07-10 (192 turns)"* — and every chip repeats both periods, with the metric measured over each, in its tooltip. The split is by **turn count**, so the halves hold equal sample sizes rather than equal calendar spans; a burst of work makes the earlier half shorter in days, which is exactly why the dates are spelled out instead of left as a vague "earlier". Trends need at least 8 turns on each side; a shorter range still reports findings, just no direction.

**When it stays quiet.** Insights are only shown once there is enough history to judge: fewer than **12** token-bearing agent turns in the current view and the whole section hides itself, so a new repo (or an over-narrow filter) sees nothing rather than advice drawn from three commits. Beyond that gate each category has its own evidence floor — a correction loop needs at least 5 corrective turns *and* at least 12% of all follow-ups; unverified turns need a repo that actually has tests — and a category that isn't triggered simply doesn't appear. A repo with healthy habits can legitimately show no insights at all. Because the categories are evidence-gated and the window is yours to choose, which cards you see changes as the slice changes and as your habits do.
...
`redact_other_repos` (default `true`) keeps the names of your OTHER projects out of this repository's commit messages: every repository aGiTrack knows about except this one is replaced with `[OTHER_REPO]` in the subject, the summary and the interaction trace (including the trace folded into your own commits in `-m` mode). Best-effort, and it never touches an ordinary word — see [Commit message format](#commit-message-format). Set it to `false`, globally or per repo, for a project whose sessions legitimately talk about its siblings (a tool developed alongside the thing it is used on), where the redaction removes context the reader actually needs.
...
`agent_background` (default `"terminal"`) is the background behind the agent. aGiTrack draws every cell of the screen itself, and by default a cell the agent left at the terminal's default colour is drawn that way too — so the session looks exactly like the terminal profile you chose, and **nothing aGiTrack does can change your background mid-session**. Set it to `"dark"` or `"light"` to override that: aGiTrack then fills the cells the agent doesn't paint (and its own status bar and popups) with that background, *and* reports that colour to the agent when it asks what background it is drawing on — so an agent that themes itself matches instead of fighting it. Also settable from `Ctrl-G` → `settings`; it applies immediately, with no restart.

`menu_key` sets the key that opens aGiTrack's command menu in interactive mode. The default is `ctrl-g`; any `ctrl-<letter>` works except keys the terminal or aGiTrack already uses (`ctrl-c` exit flow, `ctrl-h` Backspace, `ctrl-i` Tab, `ctrl-j`/`ctrl-m` Enter). An invalid value falls back to `ctrl-g`, so a typo can never lock you out of the menu. The status line and aGiTrack's messages show whichever key is configured.
