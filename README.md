...
**Each card also carries its own trend.** Every category reduces to a single number where *lower is better*; aGiTrack recomputes it on the earlier and later half of whatever range you're viewing and shows the movement as a chip — `↓ 34% vs earlier` (improving), `↑ 22% vs earlier` (worsening), or `→ steady` inside a ±20% noise band. A habit that was a problem early in the range and has since dropped below its threshold is promoted to a green **"improved"** card instead of quietly disappearing, so a win is visible rather than merely being the absence of a warning. Trends need at least 8 turns on each side; a shorter range still reports findings, just no direction.
