...
It is a **zoom**, not a wall of text. The same story at three magnifications, and you stop wherever you like:

1. **1x, the whole thing.** A title, a tagline, and the project as one bar: five parts, each sized by how much happened in it. Click one to zoom into it.
2. **5x, the five parts.** Each with its span, its size, and a sparkline of the work inside it.
3. **20x, moment by moment.** When it happened, what kind of moment it was (turning point, feature, fix, refactor, milestone, experiment), one line of what happened, and its size drawn rather than described: a bar for the lines it moved, a dot per commit, its token cost.
4. **All the way in.** The telling, then **what you asked for**: the actual prompts you typed at the pivotal moments, each with a sentence on what you were working out. Those quotes are never written by the agent; they are read straight out of the `# Interaction Trace` in your commits, and a commit the model cannot point to is dropped. Under that, the commits themselves, each opening on its commit message with a button to flip to the file changes.

Press **write the story** and your backend agent does two small readings: one over the **whole timeline**, which gives you the five parts above (always complete, however long the project is), and one over the **most recent stretch**, which gives you moments you can open. About twenty seconds. **A moment is written out the first time you zoom into it**, one call over that chapter's own material, and **go further back** tells the stretch before that when you want it, so you only pay for depth where you actually read. Everything is stored per branch in `.agitrack/story.json` and costs nothing to reopen. When new commits arrive, **add the N new commits** continues the story instead of retelling it; **go further back** reaches deeper into old history. Tell it `plainly`, `playfully` or `epically`, pick which backend and model tells it (the same setting the learn page uses), and read it newest-first with **play it**, `+`/`-` to zoom, `j`/`k` to move, or by linking straight to a level (`#z2`) or a moment (`#ch-...`).
