...
- The interaction trace records only each turn's **final** agent reply by default. The opt-in `full_agent_messages` config option (per-repo `.agitrack/config.json`, default off, mirroring `trace_turn_limit`) instead records every user-facing message the agent sent during the turn, each as its own `## Agent` block. The full list lives on `SessionTurn.agent_messages` (always populated by the parsers, cheap); the commit engine chooses all-vs-final via `_agent_messages_for`. Only conversational replies are ever included — never tool calls, tool results, reasoning, or file edits — and the trace falls back to `final_response` when a backend didn't recover the full list.
