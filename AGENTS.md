...
- `agitrack --mode json --ui-bridge` runs a long-lived **bidirectional JSON-RPC bridge** over stdin/stdout (`agitrack/shell/bridge.py`: `BridgeServer` + `BridgeUI`) for GUI drivers, primarily the VSCode extension in `editors/vscode/`. The driver sends `prompt`/`command`/`answer`/`exit` lines; aGiTrack streams `ready`/`response`/`commit`/`no_changes`/`notice`/`error`/`turn-complete`/`bye`, plus `ask` events (`kind`: select/multiselect/input/confirm) when it needs interactive input. Bridge mode is interactive even though stdin isn't a TTY: `AgitrackShell` injects `BridgeUI` into `AgitrackActions.ui`, so the untracked-file review and user-commit-message prompts (and the summarizer-model prompt) are asked of the editor instead of `input()`. `BridgeUI._ask` blocks on a thread-safe queue until the matching `answer` arrives, so menus/confirms work without a terminal. The terminal/`--prompt` paths are unchanged (`ui` defaults to `None`). `_emit`/notices route through the one lock-serialized bridge stdout channel. The TypeScript extension is not exercised by the Python gate (compile/package/F5 only); `--ui-bridge` itself is covered in `tests/test_bridge.py`.
