...
- `sessions` opens an interactive menu of the live concurrent sessions to switch between them, start a new one (in its own worktree), or stop one. Each entry shows whether the session is `running` (a turn is in flight / it produced output recently) or `idle`. Typed forms: `sessions new`, `sessions <n>` (switch to the n-th).
...
- **VSCode extension version is locked to the CLI version.** `pyproject.toml` is the single source of truth; `scripts/sync_vscode_version.py` stamps `editors/vscode/package.json` (and `package-lock.json`) to match, enforced by `tests/test_version_sync.py`. Both release workflows (`release-patch.yml` on merge to main, `release-tag.yml` on a `v*` tag) run the sync, commit it with the bump, and — when the `VSCE_TOKEN` repo secret is set — publish the extension to the Marketplace at the matching version so a CLI release and its extension move together. `agitrack --version` (cheap, no repo/privacy side effects) lets the extension detect a CLI that has self-updated past it and prompt the user to update the extension.
